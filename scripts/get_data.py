...
def fill_icustay(row):
    ...
    stay = opts[(opts['intime'] <= charttime)].sort_values(
        by='intime', ascending=True)
...
df['icustay_id'] = df[df.category.isin(
    ['Discharge summary', 'Physician ', 'Nursing', 'Nursing/other'])].apply(fill_icustay, axis=1)

df = pd.merge(df, acuities.drop(
    columns=['subject_id', 'hadm_id']), on='icustay_id', how='left')
